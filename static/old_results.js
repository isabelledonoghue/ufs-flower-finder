document.addEventListener('DOMContentLoaded', () => {
    ...
    async function populateTable(data) {
        ...
        for (const item of data) {
            ...
            // // format flowerImage url correctly (remove)
            // if (item.flowerImage.includes('&amp;')) {
            //     item.flowerImage = item.flowerImage.replace(/&amp;/g, '&');
            //     console.log("converted url", item.flowerImage);
            // }

            // // format rgb colors
            // if (item.color.includes('rgb')) {
            //     const rgbValues = item.color.match(/\d+/g).map(Number); // extract rgb values
            //     item.color = namer(rgbValues, { format: 'hex' })
            //     console.log("converted rgb color", item.color);
            // }
